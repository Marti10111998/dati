...
const dataUrl = 'https://raw.githubusercontent.com/Marti10111998/dati/refs/heads/main/transformed_data.json';

const layer = new HeatmapLayer({
  id: 'HeatmapLayer',
  data: dataUrl,
  aggregation: 'SUM',
  getPosition: d => d.COORDINATES, // Assumes data has a COORDINATES field as [lng, lat]
  getWeight: d => d.RACKS || 1,   // Use the RACKS field or default to 1
  radiusPixels: 100
});

new Deck({
  initialViewState: {
    longitude: 9.19, // Centered around Milan
    latitude: 45.486,
    zoom: 12
  },
  controller: true,
...
type DataPoint = {
  ...
  RACKS?: number;                // Weight field
};

const dataUrl = 'https://raw.githubusercontent.com/Marti10111998/dati/refs/heads/main/transformed_data.json';

const layer = new HeatmapLayer<DataPoint>({
  id: 'HeatmapLayer',
  data: dataUrl,
  aggregation: 'SUM',
  getPosition: (d: DataPoint) => d.COORDINATES,
  getWeight: (d: DataPoint) => d.RACKS || 1, // Use RACKS field or default to 1
  radiusPixels: 100
});
